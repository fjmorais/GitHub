...
WHERE granteedatabaseprincipal.name = @user
...
/****** Object:  Login [DATABRICKS]    Script Date: 11/09/2020 12:39:54 ******/
/****** Object:  User [mobilidade]    Script Date: 11/09/2020 12:42:07 ******/
CREATE USER DATABRICKS FOR LOGIN DATABRICKS WITH DEFAULT_SCHEMA=[dbo]
GO
...
set @script = 'DATABRICKS'

SELECT @@ServerName,roles.name AS role_name
    , roles.principal_id
	...
	--, N'ALTER ROLE ' + QUOTENAME(roles.name) + N' ADD MEMBER ' + QUOTENAME(@script) AS ScriptCopia
...
--WHERE memberdatabaseprincipal.name = @user
...
ALTER ROLE [db_datareader] ADD MEMBER [DATABRICKS]
ALTER ROLE [db_datawriter] ADD MEMBER [DATABRICKS]
ALTER ROLE [db_ddladmin] ADD MEMBER [DATABRICKS]
