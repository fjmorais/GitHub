...
CREATE view [vw_max_ChangedDate_tb_enderecosClb]

WITH SCHEMABINDING

as

select id,ChangedDate as LastDateInserted from [dbo].[cdc_tracking_tb_enderecosClb]
GO

create unique clustered index idx_LastDateInserted on vw_max_ChangedDate_tb_enderecosClb (id,LastDateInserted)

select top 1 LastDateInserted from vw_max_ChangedDate_tb_enderecosClb WITH(NOEXPAND)
order by 1 desc



CREATE CLUSTERED INDEX IDX_ChangedDate ON dba.dbo.cdc_tracking_tb_enderecosClb (ChangedDate)
